...
-export([string/1, file/1, bin/1, binAsBin/1, digest2str/1,get_length/2,open/0]).
...
open()->
	{ok,IoDevice} = file:open("file.txt1",[read,raw,{read_ahead,1}]),
	io:format("Size:~p ~n",[get_length(IoDevice,0)]),
	file:close(IoDevice).

get_length(IoDevice,Acc) ->
	io:format("count: ~p ~n",[Acc]),
	case file:read_line(IoDevice) of
		{ok,"Content-Length:"++RealSize} ->	[H|_T]=string:tokens(RealSize,"\n"), list_to_integer(H);
		{ok,_} -> get_length(IoDevice,Acc+1);
		{error,_} -> 0;
		eof -> 0
	end.